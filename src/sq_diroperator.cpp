...
//		connect(this, SIGNAL(fileHighlighted(const KFileItem*)), this, SLOT(slotSelected(const KFileItem*)));
	}
...
//		connect(this, SIGNAL(fileSelected(const KFileItem*)), this, SLOT(slotSelected(const KFileItem*)));
	}
...
void SQ_DirOperator::slotSelected(const KFileItem *item)
{
	if(!item) return;

	QString str = " " + item->timeString() + " ";
	QPixmap px = KMimeType::pixmapForURL(item->url(), 0, KIcon::Desktop, 16);
	sqSBcurFileInfo->setText(str);
	sqSBfileIcon->setPixmap(px);
	sqSBfileName->setText("  " + item->text() + ((item->isDir())?"":(" (" + KIO::convertSize(item->size()) + ")")));
}
...
{
	static QString str;
	str.sprintf(" Total %d (%d dirs, %d files) ", new_view->count(), files, dirs);
