...
typedef unsigned char *(*PixopsLineFunc) (int *weights, int n_x, int n_y,
				   unsigned char *dest, int dest_x, unsigned char *dest_end, int dest_channels, int dest_has_alpha,
				   unsigned char **src, int src_channels, int src_has_alpha,
				   int x_init, int x_step, int src_width,
				   int check_size, unsigned int color1, unsigned int color2);

typedef void (*PixopsPixelFunc) (unsigned char *dest, int dest_x, int dest_channels, int dest_has_alpha,
				 int src_has_alpha, int check_size, unsigned int color1,
				 unsigned int color2,
      ...
      else if (src_channels == 4)
	{
	  if (dest_channels == 3)
	  ...
	  else
	    {
	      for (j=0; j < (render_x1 - render_x0); j++)
		{
		  const unsigned char *p = src + (x >> SCALE_SHIFT) * 4;
		  unsigned int *p32;

		  p32 = (unsigned int *) dest;
		  *p32 = *((unsigned int *) p);
...
static void
pixops_composite_nearest (unsigned char        *dest_buf,
			  int            render_x0,
			  int            render_y0,
			  int            render_x1,
			  int            render_y1,
...
static void
pixops_composite_color_nearest (unsigned char        *dest_buf,
				int            render_x0,
				int            render_y0,
				int            render_x1,
				int            render_y1,
...
static void
composite_pixel (unsigned char *dest, int dest_x, int dest_channels, int dest_has_alpha,
		 int src_has_alpha, int check_size, unsigned int color1, unsigned int color2,
		 unsigned int r, unsigned int g, unsigned int b, unsigned int a)
{
  if (dest_has_alpha)
    {
  ...
  while (dest < dest_end)
    {
      ...
      for (i=0; i<n_y; i++)
	{
	  unsigned char *q = src[i] + x_scaled * src_channels;
...
static void
composite_pixel_color (unsigned char *dest, int dest_x, int dest_channels, int dest_has_alpha,
		       int src_has_alpha, int check_size, unsigned int color1, unsigned int color2,
		       unsigned int r, unsigned int g, unsigned int b, unsigned int a)
{
...
static void
scale_pixel (unsigned char *dest, int dest_x, int dest_channels, int dest_has_alpha,
	     int src_has_alpha, int check_size, unsigned int color1, unsigned int color2,
	     unsigned int r, unsigned int g, unsigned int b, unsigned int a)
{
  if (src_has_alpha)
...
process_pixel (int *weights, int n_x, int n_y,
	       unsigned char *dest, int dest_x, int dest_channels, int dest_has_alpha,
	       unsigned char **src, int src_channels, int src_has_alpha,
...
static void
pixops_process (unsigned char         *dest_buf,
		int             render_x0,
		int             render_y0,
		int             render_x1,
		int             render_y1,
		...
		PixopsPixelFunc pixel_func)
{
  ...
  unsigned char **line_bufs = (unsigned char **)calloc (filter->n_y, sizeof(unsigned char *));
  ...
  for (i = 0; i < (render_y1 - render_y0); i++)
    {
      ...
      unsigned char *new_outbuf;
      unsigned int tcolor1, tcolor2;
      
      unsigned char *outbuf = dest_buf + dest_rowstride * i;
      unsigned char *outbuf_end = outbuf + dest_channels * (render_x1 - render_x0);
	...
	{
	  if (y_start <  0)
	    line_bufs[j] = (unsigned char *)src_buf;
	  else if (y_start < src_height)
	    line_bufs[j] = (unsigned char *)src_buf + src_rowstride * y_start;
	  else
	    line_bufs[j] = (unsigned char *)src_buf + src_rowstride * (src_height - 1);
...
tile_make_weights (PixopsFilter *filter, double x_scale, double y_scale, double overall_alpha)
{
  ...
  filter->n_x = n_x;
  filter->n_y = n_y;
  filter->weights = (int *)calloc (SUBSAMPLE * SUBSAMPLE * n_x * n_y, sizeof(int));
...
bilinear_make_fast_weights (PixopsFilter *filter, double x_scale, double y_scale, double overall_alpha)
{
  ...
  x_weights = (double *)calloc (n_x, sizeof(double));
  y_weights = (double *)calloc (n_y, sizeof(double));
...
void
pixops_composite_color (unsigned char         *dest_buf,
			int             render_x0,
			int             render_y0,
			int             render_x1,
			int             render_y1,
 ...
 **/
void
pixops_composite (unsigned char        *dest_buf,
		  int            render_x0,
		  int            render_y0,
		  int            render_x1,
		  int            render_y1,
...
void
pixops_scale (unsigned char        *dest_buf,
	      int            render_x0,
	      int            render_y0,
	      int            render_x1,
	      int            render_y1,
